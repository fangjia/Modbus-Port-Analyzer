...
:set_filter_2MW
    echo.
    echo 選擇抓包濾鏡條件...
    echo 1. ALL INFOMATION
    echo 2. 僅限主機 IP (host 192.168.100.150)
    echo 3. 主機 IP 與 TCP 埠 (host 192.168.100.150 and tcp port 33999)
    echo 4. 主機 IP 與 TCP 埠 (host 192.168.100.150 and host 192.168.100.29)
    ...
    if "!FILTER_CHOICE!"=="" (
        ...
        set CAPTURE_FILTER=host 192.168.100.150 and host 192.168.100.29
    ) ELSE (
        echo 錯誤：未輸入正確的濾鏡編號
        pause
        exit /b 1
    )
echo DEBUG: CAPTURE_FILTER set to !CAPTURE_FILTER! for choice !FILTER_CHOICE!
