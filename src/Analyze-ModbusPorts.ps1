...
Write-Host "[1/4] 正在提取所有對 Unit $UnitId 進行查詢的 Source Ports..." -ForegroundColor Yellow
...
Write-Host "      ?? 統計: 共使用 $($validUnitPorts.Count) 個獨立的 Source Ports。" -ForegroundColor Green

# 步驟2: 提取通訊階段有過 FIN 或 RST 的 Ports (代表連線被正常關閉或重置)
Write-Host "[2/4] 正在建立並掃描由 FIN 或 RST 終止的已釋放連線池..." -ForegroundColor Yellow
...
# 步驟3: 兩邊資料進行比對，寫入最終狀態
Write-Host "[3/4] 正在進行交叉比對並匯出狀態清單至 CSV..." -ForegroundColor Yellow
$releasedCount = 0
$unclosedCount = 0
...
# 步驟4: 提取 RST 封包資訊
Write-Host "[4/4] 正在提取 RST (Connection Reset) 封包資訊..." -ForegroundColor Yellow
...
if ($rstCount -gt 0) {
    ...
    Write-Host "      ?? 統計: 找到 $rstCount 筆 RST 封包，已獨立匯出至文字檔" -ForegroundColor Green
} else {
    Write-Host "      ?? 統計: 找不到符合過濾條件的 RST 封包" -ForegroundColor Green
}
...
?? 參與查詢的總 Port 數 : $($releasedCount + $unclosedCount)
  ?? 已正確釋放  (FIN/RST): $releasedCount
  ? 未釋放 (無交握中斷記錄): $unclosedCount
  ? RST 封包總數            : $rstCount (過濾條件: tcp.connection.rst)

"@

if ($unclosedCount -gt 0) {
    $reportContent += "`n?? 警告：發現 $unclosedCount 個未正確關閉可能洩漏的 Ports：`n"
    # 取出未釋放的 ports
    $unclosedPortsList = @()
...
if ($rstCount -gt 0) {
    $reportContent += "`n? 找到 $rstCount 筆符合條件的 RST 封包，以下為最近的幾筆摘錄 (完整請參考 _RST_info.txt)：`n"
    $reportContent += "---------------------------------------------`n"
    $maxRstPreview = 10
...
Write-Host "  ?? 已正確釋放  (FIN/RST): $releasedCount" -ForegroundColor Green
Write-Host "  ? 未釋放 (無交握中斷記錄): $unclosedCount" -ForegroundColor Red

if ($unclosedCount -gt 0) {
    Write-Host "`n?? 警告：發現 $unclosedCount 個未正確關閉可能洩漏的 Ports：" -ForegroundColor Red
...
if ($rstCount -gt 0) {
    Write-Host "`n? 找到 $rstCount 筆符合條件的 RST 封包，以下為最近的幾筆摘錄 (完整請參考 _RST_info.txt)：" -ForegroundColor Yellow
    Write-Host "---------------------------------------------" -ForegroundColor Yellow
    $maxRstPreview = 12
    $previewList = $rstPackets
    if ($rstPackets -is [array] -and $rstPackets.Count -gt $maxRstPreview) {
...
if ($rstCount -gt 0) {
    Write-Host "? 獨立 RST 資訊 已匯出至: $rstFile" -ForegroundColor Yellow
}
Write-Host "=============================================" -ForegroundColor Cyan
