...
* **`Run-Analyzer.bat`**：使用者的主要啟動入口。雙擊執行後會自動調用 `src/` 裡的程式，並開始分析。
* **`src/Analyze-ModbusPorts.ps1`**：核心分析腳本程式。負責啟動 tshark 過濾器、構建終端機互動選單 (UI)、以及交叉比對釋放邏輯、產出報告。
* **`data/`**：這就是未來您的操作舞台！
  * 請將側錄好的封包檔 (`.pcap` 或 `.pcapng`) 丟進這個資料夾。
  * 腳本執行完成後產出的 `.csv` 分析報告，以及 `_Summary.txt` 文字總結報告，也會整齊地存放在這個裡面。
